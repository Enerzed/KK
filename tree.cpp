...
bool Tree::AddSymbol(const std::string& name, TypeObject kind, TypeData type, int arraySize)
{
    if (FindSymbolCurrent(name) != nullptr)
        return false;
    Symbol sym{ name, kind, type, arraySize, false };
...
Tree* Tree::Include(TypeLex lex, TypeObject kind, TypeData type)
{
    if (AddSymbol(lex, kind, type))
    {
        Symbol* sym = FindSymbolCurrent(lex);
        return reinterpret_cast<Tree*>(sym);
    }
    else
    {
...
void Tree::SetObject(Tree* node, TypeObject kind)
{
    Symbol* sym = reinterpret_cast<Symbol*>(node);
    if (sym) sym->kind = kind;
}

Tree* Tree::FindUp(TypeLex lex)
{
    Symbol* sym = FindSymbol(lex);
    if (!sym)
        scanner->PrintError("Identifier not declared", lex);
    return reinterpret_cast<Tree*>(sym);
}